  ...
  useEffect(() => {
    const fetchContracts = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const approvedContracts = data.contracts.filter(c => c.status === 'approved');
          setContracts(approvedContracts.length > 0 ? [approvedContracts[0]] : []);
          console.log("MyContracts - Contract data in state (for FormContracts):");
